...
/**
 * @brief Simulates parallel pass of the graph by ants of colony
 *
 */
void AntAlgorithm::RunAntsParallel() {
  ...
  Threads threads;
  for (size_t i = 0; i < graph_.GetGraph().size(); ++i) {
    threads.emplace_back(std::thread(&AntAlgorithm::ParallelBypass, this, i));
  }
...
void AntAlgorithm::ParallelBypass(size_t vertex) {
  std::unique_ptr<Ant> ant = std::make_unique<Ant>(graph_, vertex);
  ant->MakeBypass(pheromones_);
  mtx_.lock();
...
void AntAlgorithm::SetParallel(const bool parallel) { parallel_ = parallel; }
